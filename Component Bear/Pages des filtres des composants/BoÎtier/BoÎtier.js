...
validateButton.addEventListener("click", () => {
 ...
 checkboxes.forEach(checkbox => {
   ...
   selectedBoitier.push({ name: productName, price: productPrice });
 });

 if (selectedBoitier.length > 0) {
 
   localStorage.setItem("selectedBoitier", JSON.stringify(selectedBoitier));

   window.location.href = "/Website Beta/Page principale/Configurateur PC.html";
 } else {
   alert("Veuillez sélectionner un boÎtier avant de valider.");
 }
});