...
validateButton.addEventListener("click", () => {
    ...
    checkboxes.forEach(checkbox => {
        ...
        selectedAlim.push({ name: productName, price: productPrice,}); 
    });

    if (selectedAlim.length > 0) {

        localStorage.setItem("selectedAlim", JSON.stringify(selectedAlim));

        window.location.href = "/Website Beta/Page principale/Configurateur PC.html";
    } else {
        alert("Veuillez sélectionner une alimentation avant de valider.");
    }
});